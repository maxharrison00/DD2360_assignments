...
int main(int argc, char **argv) {
  ...
  cusparseSpMatDescr_t matA;       // Sparse matrix descriptor for generic API
  ...
  //@@ Insert code to call cusparse api to create the sparse matrix descriptor
  //(generic API)
  cusparseCheck(cusparseCreateCsr(&matA, dimX, dimX, nzv, ARowPtr, AColIndx, A,
                                  CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                  CUSPARSE_INDEX_BASE_ZERO, CUDA_R_64F));

  //@@ Insert code to create dense vector descriptors
  ...
  //@@ Insert code to call cusparse api to get the buffer size needed by SpMV
  cusparseCheck(cusparseSpMV_bufferSize(
      cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, matA, vecTemp,
      &zero, vecTmp, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize));
  ...
  for (int it = 0; it < nsteps; ++it) {
    //@@ Insert code to call cusparse api to compute the SpMV (sparse
    //matrix-vector multiplication)
    //@@ This calculation corresponds to: tmp = 1 * A * temp + 0 * tmp
    cusparseCheck(cusparseSpMV(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                               &one, matA, vecTemp, &zero, vecTmp, CUDA_R_64F,
    ...
    //@@ Insert code to call cublas api to compute the norm of the vector using
    //cuBLAS
  ...
  one = -1;
  //@@ Insert the code to call cublas api to compute the difference between the
  //exact solution
  ...
  //@@ Insert the code to destroy the descriptors
