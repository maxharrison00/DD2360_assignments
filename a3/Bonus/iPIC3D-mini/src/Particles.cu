...
__global__ void mover_PC_kernel(
    ...
    FPfield *Ex, FPfield *Ey, FPfield *Ez, FPfield *Bxn, FPfield *Byn,
    FPfield *Bzn, FPfield *XN_flat, FPfield *YN_flat, FPfield *ZN_flat, int nop,
    int nxn, int nyn, int nzn, FPpart dt_sub_cycling, FPpart dto2,
    FPpart qomdt2, FPfield invdx, FPfield invdy, FPfield invdz, FPfield invVOL,
    FPfield xStart, FPfield yStart, FPfield zStart, int NiterMover, FPfield Lx,
    FPfield Ly, FPfield Lz, bool PERIODICX, bool PERIODICY, bool PERIODICZ) {
  ...
  // Calculate average velocity iteratively
  for (int innter = 0; innter < NiterMover; innter++) {
    ...
    // Calculate weights
    // Access flattened 3D arrays: index = ix + iy*nxn + iz*nxn*nyn
    xi[0] = x[i] - XN_flat[(ix - 1) + iy * nxn + iz * nxn * nyn];
    eta[0] = y[i] - YN_flat[ix + (iy - 1) * nxn + iz * nxn * nyn];
    zeta[0] = z[i] - ZN_flat[ix + iy * nxn + (iz - 1) * nxn * nyn];
    xi[1] = XN_flat[ix + iy * nxn + iz * nxn * nyn] - x[i];
    eta[1] = YN_flat[ix + iy * nxn + iz * nxn * nyn] - y[i];
    zeta[1] = ZN_flat[ix + iy * nxn + iz * nxn * nyn] - z[i];
    ...
    for (int ii = 0; ii < 2; ii++)
      for (int jj = 0; jj < 2; jj++)
        for (int kk = 0; kk < 2; kk++) {
          int idx = (ix - ii) + (iy - jj) * nxn + (iz - kk) * nxn * nyn;
          Exl += weight[ii][jj][kk] * Ex[idx];
          Eyl += weight[ii][jj][kk] * Ey[idx];
          Ezl += weight[ii][jj][kk] * Ez[idx];
          Bxl += weight[ii][jj][kk] * Bxn[idx];
          Byl += weight[ii][jj][kk] * Byn[idx];
          Bzl += weight[ii][jj][kk] * Bzn[idx];
        }
    ...
    FPpart ut = u[i] + qomdt2 * Exl;
    FPpart vt = v[i] + qomdt2 * Eyl;
    FPpart wt = w[i] + qomdt2 * Ezl;
...
int mover_PC(struct particles *part, struct EMfield *field, struct grid *grd,
             struct parameters *param) {
  std::cout << "***  GPU MOVER with SUBCYCLING " << param->n_sub_cycles
            << " - species " << part->species_ID << " ***" << std::endl;
  ...
  FPfield *d_Ex, *d_Ey, *d_Ez, *d_Bxn, *d_Byn, *d_Bzn;
  ...
  // Allocate device memory for fields (flattened 3D arrays)
  ...
  // Flatten and copy field data to device
  ...
  for (int i_sub = 0; i_sub < part->n_sub_cycles; i_sub++) {
    mover_PC_kernel<<<blocksPerGrid, threadsPerBlock>>>(
        d_x, d_y, d_z, d_u, d_v, d_w, d_Ex, d_Ey, d_Ez, d_Bxn, d_Byn, d_Bzn,
        d_XN, d_YN, d_ZN, nop, nxn, nyn, nzn, dt_sub_cycling, dto2, qomdt2,
        grd->invdx, grd->invdy, grd->invdz, grd->invVOL, grd->xStart,
        grd->yStart, grd->zStart, part->NiterMover, grd->Lx, grd->Ly, grd->Lz,
